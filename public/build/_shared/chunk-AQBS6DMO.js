...
function GoogleMapFunctional(_a) {
  ...
  var _c3 = (0, import_react.useState)(null), centerChangedListener = _c3[0], setCenterChangedListener = _c3[1];
...
function useLoadScript(_a) {
  var _b = _a.id, id = _b === void 0 ? defaultLoadScriptProps.id : _b, _c3 = _a.version, version = _c3 === void 0 ? defaultLoadScriptProps.version : _c3, nonce = _a.nonce, googleMapsApiKey = _a.googleMapsApiKey, googleMapsClientId = _a.googleMapsClientId, language = _a.language, region = _a.region, libraries = _a.libraries, preventGoogleFontsLoading = _a.preventGoogleFontsLoading, channel = _a.channel, mapIds = _a.mapIds, authReferrerPolicy = _a.authReferrerPolicy;
...
function DrawingManagerFunctional(_a) {
  ...
  var map = (0, import_react.useContext)(MapContext);
  var _b = (0, import_react.useState)(null), instance = _b[0], setInstance = _b[1];
  var _c3 = (0, import_react.useState)(null), circlecompleteListener = _c3[0], setCircleCompleteListener = _c3[1];
...
function MarkerFunctional(_a) {
  ...
  var map = (0, import_react.useContext)(MapContext);
  var _b = (0, import_react.useState)(null), instance = _b[0], setInstance = _b[1];
  var _c3 = (0, import_react.useState)(null), dblclickListener = _c3[0], setDblclickListener = _c3[1];
  ...
  var _s3 = (0, import_react.useState)(null), flatChangedListener = _s3[0], setFlatChangedListener = _s3[1];
  ...
  function() {
    ...
    ClusterIcon2.prototype.show = function() {
      ...
      if (this.div && this.center) {
        ...
        if ((_c3 = this.sums) === null || _c3 === void 0 ? void 0 : _c3.text)
...
var Clusterer = (
  ...
  function() {
    ...
    Clusterer2.prototype.onRemove = function() {
      ...
      for (var _b = 0, _c3 = this.clusters; _b < _c3.length; _b++) {
        var cluster = _c3[_b];
        cluster.remove();
      }
      this.clusters = [];
    ...
    Clusterer2.prototype.resetViewport = function(optHide) {
      ...
      this.clusters = [];
      for (var _b = 0, _c3 = this.markers; _b < _c3.length; _b++) {
        var marker = _c3[_b];
        marker.isAdded = false;
        if (optHide) {
          marker.setMap(null);
...
function MarkerClustererFunctional(props) {
  ...
  var _c3 = (0, import_react.useState)(null), clusteringBeginListener = _c3[0], setClusteringBeginListener = _c3[1];
...
function InfoBoxFunctional(_a) {
  ...
  var map = (0, import_react.useContext)(MapContext);
  var _b = (0, import_react.useState)(null), instance = _b[0], setInstance = _b[1];
  var _c3 = (0, import_react.useState)(null), closeClickListener = _c3[0], setCloseClickListener = _c3[1];
...
var KDBush2 = class {
  ...
  static from(data) {
    ...
    const version = versionAndType >> 4;
    if (version !== VERSION2) {
      throw new Error(`Got v${version} data when expected v${VERSION2}.`);
    }
    const ArrayType = ARRAY_TYPES2[versionAndType & 15];
    if (!ArrayType) {
      throw new Error("Unrecognized array type.");
    }
    const [nodeSize] = new Uint16Array(data, 2, 1);
    const [numItems] = new Uint32Array(data, 4, 1);
    return new KDBush2(numItems, nodeSize, ArrayType, data);
  }
  /**
   * Creates an index that will hold a given number of items.
   * @param {number} numItems
  ...
  constructor(numItems, nodeSize = 64, ArrayType = Float64Array, data) {
    ...
    this.ArrayType = ArrayType;
    this.IndexArrayType = numItems < 65536 ? Uint16Array : Uint32Array;
    const arrayTypeIndex = ARRAY_TYPES2.indexOf(this.ArrayType);
    const coordsByteSize = numItems * 2 * this.ArrayType.BYTES_PER_ELEMENT;
    const idsByteSize = numItems * this.IndexArrayType.BYTES_PER_ELEMENT;
    const padCoords = (8 - idsByteSize % 8) % 8;
    if (arrayTypeIndex < 0) {
    ...
    if (data && data instanceof ArrayBuffer) {
      this.data = data;
      this.ids = new this.IndexArrayType(this.data, HEADER_SIZE2, numItems);
      this.coords = new this.ArrayType(this.data, HEADER_SIZE2 + idsByteSize + padCoords, numItems * 2);
      this._pos = numItems * 2;
      this._finished = true;
    } else {
      this.data = new ArrayBuffer(HEADER_SIZE2 + coordsByteSize + idsByteSize + padCoords);
      this.ids = new this.IndexArrayType(this.data, HEADER_SIZE2, numItems);
      this.coords = new this.ArrayType(this.data, HEADER_SIZE2 + idsByteSize + padCoords, numItems * 2);
      this._pos = 0;
      this._finished = false;
      new Uint8Array(this.data, 0, 2).set([219, (VERSION2 << 4) + arrayTypeIndex]);
      new Uint16Array(this.data, 2, 1)[0] = nodeSize;
      new Uint32Array(this.data, 4, 1)[0] = numItems;
    }
  }
  ...
  within(qx, qy, r) {
    ...
    while (stack.length) {
      ...
      if (right - left <= nodeSize) {
        for (let i = left; i <= right; i++) {
          if (sqDist2(coords[2 * i], coords[2 * i + 1], qx, qy) <= r2)
            result.push(ids[i]);
        }
        continue;
      }
...
function select2(ids, coords, k, left, right, axis) {
  while (right > left) {
    if (right - left > 600) {
      ...
      const newLeft = Math.max(left, Math.floor(k - m * s / n + sd));
      const newRight = Math.min(right, Math.floor(k + (n - m) * s / n + sd));
      select2(ids, coords, k, newLeft, newRight, axis);
    }
    const t = coords[2 * k + axis];
    let i = left;
    let j = right;
    swapItem2(ids, coords, left, k);
    if (coords[2 * right + axis] > t)
      swapItem2(ids, coords, left, right);
    while (i < j) {
      swapItem2(ids, coords, i, j);
      i++;
      j--;
      while (coords[2 * i + axis] < t)
        i++;
    ...
    if (coords[2 * left + axis] === t)
      swapItem2(ids, coords, left, j);
    else {
      j++;
      swapItem2(ids, coords, j, right);
    }
    if (j <= k)
      left = j + 1;
    if (k <= j)
...
function swapItem2(ids, coords, i, j) {
  swap2(ids, i, j);
...
var fround2 = Math.fround || ((tmp) => (x) => {
  tmp[0] = +x;
  return tmp[0];
})(new Float32Array(1));
var OFFSET_ZOOM2 = 2;
var OFFSET_ID2 = 3;
var OFFSET_PARENT2 = 4;
var OFFSET_NUM2 = 5;
var OFFSET_PROP2 = 6;
var Supercluster2 = class {
  constructor(options) {
  ...
  load(points) {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      if (!p.geometry)
        continue;
      const [lng, lat] = p.geometry.coordinates;
      const x = fround2(lngX2(lng));
      const y = fround2(latY2(lat));
      data.push(
        x,
        y,
        // projected point coordinates
  ...
  getClusters(bbox, zoom) {
    ...
    const tree = this.trees[this._limitZoom(zoom)];
    const ids = tree.range(lngX2(minLng), latY2(maxLat), lngX2(maxLng), latY2(minLat));
    const data = tree.data;
    const clusters = [];
    for (const id of ids) {
      const k = this.stride * id;
      clusters.push(data[k + OFFSET_NUM2] > 1 ? getClusterJSON2(data, k, this.clusterProps) : this.points[data[k + OFFSET_ID2]]);
    }
    return clusters;
  }
  getChildren(clusterId) {
    ...
    for (const id of ids) {
      const k = id * this.stride;
      if (data[k + OFFSET_PARENT2] === clusterId) {
        children.push(data[k + OFFSET_NUM2] > 1 ? getClusterJSON2(data, k, this.clusterProps) : this.points[data[k + OFFSET_ID2]]);
      }
    }
    if (children.length === 0)
      throw new Error(errorMsg);
  ...
  _createTree(data) {
    const tree = new KDBush2(data.length / this.stride | 0, this.options.nodeSize, Float32Array);
    for (let i = 0; i < data.length; i += this.stride)
      tree.add(data[i], data[i + 1]);
    tree.finish();
    tree.data = data;
  ...
  _addTileFeatures(ids, data, x, y, z2, tile) {
    for (const i of ids) {
      const k = i * this.stride;
      const isCluster = data[k + OFFSET_NUM2] > 1;
      let tags, px, py;
      if (isCluster) {
        tags = getClusterProperties2(data, k, this.clusterProps);
        px = data[k];
        py = data[k + 1];
      } else {
        const p = this.points[data[k + OFFSET_ID2]];
        tags = p.properties;
        const [lng, lat] = p.geometry.coordinates;
        px = lngX2(lng);
        py = latY2(lat);
      ...
      let id;
      if (isCluster || this.options.generateId) {
        id = data[k + OFFSET_ID2];
      } else {
        id = this.points[data[k + OFFSET_ID2]].id;
      }
      if (id !== void 0)
        f.id = id;
      tile.features.push(f);
  ...
  _cluster(tree, zoom) {
    ...
    const stride = this.stride;
    for (let i = 0; i < data.length; i += stride) {
      if (data[i + OFFSET_ZOOM2] <= zoom)
        continue;
      data[i + OFFSET_ZOOM2] = zoom;
      const x = data[i];
      const y = data[i + 1];
      const neighborIds = tree.within(data[i], data[i + 1], r);
      const numPointsOrigin = data[i + OFFSET_NUM2];
      let numPoints = numPointsOrigin;
      for (const neighborId of neighborIds) {
        const k = neighborId * stride;
        if (data[k + OFFSET_ZOOM2] > zoom)
          numPoints += data[k + OFFSET_NUM2];
      }
      if (numPoints > numPointsOrigin && numPoints >= minPoints) {
        let wx = x * numPointsOrigin;
        let wy = y * numPointsOrigin;
        ...
        for (const neighborId of neighborIds) {
          ...
          if (data[k + OFFSET_ZOOM2] <= zoom)
            continue;
          data[k + OFFSET_ZOOM2] = zoom;
          const numPoints2 = data[k + OFFSET_NUM2];
          wx += data[k] * numPoints2;
          wy += data[k + 1] * numPoints2;
          data[k + OFFSET_PARENT2] = id;
          if (reduce2) {
            if (!clusterProperties) {
              clusterProperties = this._map(data, i, true);
              clusterPropIndex = this.clusterProps.length;
  ...
  _map(data, i, clone) {
    if (data[i + OFFSET_NUM2] > 1) {
      const props = this.clusterProps[data[i + OFFSET_PROP2]];
      return clone ? Object.assign({}, props) : props;
    }
    const original = this.points[data[i + OFFSET_ID2]].properties;
    const result = this.options.map(original);
    return clone && result === original ? Object.assign({}, result) : result;
  }
};
function getClusterJSON2(data, i, clusterProps) {
  return {
    type: "Feature",
    id: data[i + OFFSET_ID2],
    properties: getClusterProperties2(data, i, clusterProps),
    geometry: {
      type: "Point",
      coordinates: [xLng2(data[i]), yLat2(data[i + 1])]
    }
  };
}
function getClusterProperties2(data, i, clusterProps) {
  const count = data[i + OFFSET_NUM2];
  const abbrev = count >= 1e4 ? `${Math.round(count / 1e3)}k` : count >= 1e3 ? `${Math.round(count / 100) / 10}k` : count;
  const propIndex = data[i + OFFSET_PROP2];
  const properties = propIndex === -1 ? {} : Object.assign({}, clusterProps[propIndex]);
  return Object.assign(properties, {
    cluster: true,
    cluster_id: data[i + OFFSET_ID2],
    point_count: count,
    point_count_abbreviated: abbrev
  });
}
...
// app/utils/getDistanceBetween2LatLongPointsInKm.ts
if (import.meta) {
  import.meta.hot = createHotContext(
    //@ts-expect-error
    "app/utils/getDistanceBetween2LatLongPointsInKm.ts"
  );
  import.meta.hot.lastModified = "1706454925364.5916";
}
function getDistanceBetween2LatLongPointsInKm(lat1, lon1, lat2, lon2) {
  const radiusOfEarthInKm = 6371;
  const dLat = degrees2Radians(lat2 - lat1);
  const dLon = degrees2Radians(lon2 - lon1);
  const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(degrees2Radians(lat1)) * Math.cos(degrees2Radians(lat2)) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
...
if (!window.$RefreshReg$ || !window.$RefreshSig$ || !window.$RefreshRuntime$) {
  ...
  window.$RefreshReg$ = (type, id) => {
    window.$RefreshRuntime$.register(type, '"app/components/ui/Map.tsx"' + id);
  };
...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1706454925344.4622";
}
var superCluster = new Supercluster2({
  radius: 40,
  maxZoom: 20
});
var formatDataToGeoJsonPoints = (markers) => {
...
var MapComponent = ({
  ...
  if (!isLoaded)
    return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)("div", { children: "Loading..." }, void 0, false, {
      fileName: "app/components/ui/Map.tsx",
      lineNumber: 66,
      columnNumber: 25
    }, this);
  return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(Map, { recenterMapOnMarkerChange, doCluster, stopMovingMap, markersCoordinates, onClick }, void 0, false, {
    fileName: "app/components/ui/Map.tsx",
    lineNumber: 67,
    columnNumber: 10
  }, this);
};
_s(MapComponent, "0WnOjCnOubliry/bcWK4XUbomlA=", false, function() {
...
var Map = ({
  ...
  _s2();
  const [centerLocation, setCenterLocation] = (0, import_react2.useState)({
    lat: 51.5072,
    lng: 0.1276
  });
  const [initialZoom, setInitialZoom] = (0, import_react2.useState)(14);
  const [zoom, setZoom] = (0, import_react2.useState)(14);
  const [bounds, setBounds] = (0, import_react2.useState)([0, 0, 0, 0]);
  const [clusters, setClusters] = (0, import_react2.useState)();
  const [firstLoad, setFirstLoad] = (0, import_react2.useState)(true);
  const mapRef = (0, import_react2.useRef)();
  const [mapClick, setMapClick] = (0, import_react2.useState)(true);
  const recenterMap = () => {
    ...
    if (markersCoordinates.length === 0 && navigator.geolocation) {
      ...
      const averageLongitude = allLongitudes.reduce((accumulator, currentValue) => accumulator + currentValue, 0) / allLongitudes.length;
      ...
      const averageLatitudes = allLatitudes.reduce((accumulator, currentValue) => accumulator + currentValue, 0) / allLatitudes.length;
      const distanceBetweenToPoints = getDistanceBetween2LatLongPointsInKm(maxLatitude, maxLongitude, minLatitude, minLongitude);
      if (markersCoordinates.length !== 1) {
        const zoom2 = Math.log2(4e4 / (distanceBetweenToPoints / 2)) - 3;
        setInitialZoom(zoom2);
      }
      setCenterLocation({
        lat: averageLatitudes,
        lng: averageLongitude
      });
    }
  };
  (0, import_react2.useEffect)(() => {
    if (doCluster && (markersCoordinates == null ? void 0 : markersCoordinates.length) && mapRef.current) {
      superCluster.load(formatDataToGeoJsonPoints(markersCoordinates));
  ...
  const handleClusterClick = ({
    ...
    (_a = mapRef.current) == null ? void 0 : _a.setZoom(expansionZoom);
    (_b = mapRef.current) == null ? void 0 : _b.panTo({
      lat,
      lng
    });
  };
  const handleBoundsChanged = () => {
    var _a;
    if (mapRef.current) {
      const bounds2 = (_a = mapRef.current.getBounds()) == null ? void 0 : _a.toJSON();
      setBounds([(bounds2 == null ? void 0 : bounds2.west) || 0, (bounds2 == null ? void 0 : bounds2.south) || 0, (bounds2 == null ? void 0 : bounds2.east) || 0, (bounds2 == null ? void 0 : bounds2.north) || 0]);
    }
  };
  const handleZoomChanged = () => {
    var _a;
    if (mapRef.current) {
      setZoom((_a = mapRef.current) == null ? void 0 : _a.zoom);
    }
  };
  ...
  return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(GoogleMap, { onLoad: handleMapLoad, onBoundsChanged: handleBoundsChanged, onZoomChanged: handleZoomChanged, zoom: initialZoom, center: centerLocation, mapContainerStyle: {
    height: "100%",
    width: "100%"
  }, onClick: async (props) => {
  ...
  }, children: doCluster ? clusters == null ? void 0 : clusters.map(({
    id,
    geometry,
    properties
  }) => {
    ...
    return cluster ? /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(MarkerF, { onClick: () => handleClusterClick({
      id,
      lat,
      lng
    }), position: {
      lat,
      lng
    }, label: getLabel(point_count) }, `cluster-${id}`, false, {
      fileName: "app/components/ui/Map.tsx",
      lineNumber: 198,
      columnNumber: 24
    }, this) : /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(MarkerF, { position: {
      lat,
      lng
    }, onClick: onClick2 }, `location-${id}`, false, {
      fileName: "app/components/ui/Map.tsx",
      lineNumber: 205,
      columnNumber: 45
    }, this);
  }) : markersCoordinates.map((markerCoordinates, index) => {
    return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(MarkerF, { position: markerCoordinates, onClick: markerCoordinates.onClick }, "mapMarker" + index, false, {
      fileName: "app/components/ui/Map.tsx",
      lineNumber: 210,
      columnNumber: 14
    }, this);
...
_s2(Map, "5xNndcqGmY535EdUymmQ5KtRZpA=");
_c2 = Map;
var _c;
var _c2;
...
@react-google-maps/api/dist/esm.js:
  ...
  ***************************************************************************** *)
*/
//# sourceMappingURL=/build/_shared/chunk-AQBS6DMO.js.map
